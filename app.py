...
def translate_to_zh(text: str) -> str:
    """
    把英文释义翻译成中文。
    如果 translator 不可用或翻译失败，则返回空字符串。
    """
    text = text.strip()
...
def ensure_meanings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "word" not in df.columns:
        raise ValueError("DataFrame 必须有 'word' 列作为单词列。")
...
def load_local_csv():
    ...
    从本地 CSV 读取：
    - 把第一列当作单词列，列名改成 'word'；
    - 第二列如果存在，当作原始 meaning 兜底；
    - 调用 ensure_meanings 统一更新 + 加中文。
    """
    try:
...
def main():
    ...
    st.write(
        "从仓库中的 `word_list.csv` 读取单词："
        "第一列作为单词，第二列（如果有）作为原始释义。"
        "程序会自动按单词更新英文释义，并尽量附上中文解释。"
    )
    ...
    with col2:
        ...
        if st.session_state.phase == "test" and questions:
            ...
            st.markdown("为每个单词选择正确的释义（英文 + 可能的中文）。")
